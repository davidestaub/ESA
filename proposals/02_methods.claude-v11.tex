...
after six edge groups without an established wavelength mapping are
excluded.
...
content hash. Six
knots per order carry each transformed coordinate. Per coordinate and
...
transit template is at most 0.25. On the real visit the leading
component correlates with the transit at $|r|=0.950$ and is vetoed. The
...
\end{equation}
where $\pi_{pg}$ is a spatial profile estimated on the same input. Each
term is the inverse-variance amplitude of that profile in the group, and
pixels shared by both orders are excluded from both curves. The
