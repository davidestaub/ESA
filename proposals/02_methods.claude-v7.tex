...
those quantities from the observed samples. Nothing is extracted, binned,
...
absolute correlation with a transit-shaped reference stays below 0.25, so
the background cannot absorb the transit. The off-trace fit also yields
...
A fit is accepted only when the weights, the objective, and the shared
spectrum have stopped changing, the first-order optimality measures are
met, and a confirmation fit at the recomputed terminal weights
reproduces the solution; the numerical gates are listed in
...
must agree to within fractions of a part per million before a solution
is accepted, and if both converge, the one with the lower robust
objective is selected. Both starts converged, at robust cycle 14.
...
the calibrations of the previous subsection draw their masks from this
complement (Appendix~\ref{app:nova}). The injected spectrum is revealed
