...
For byte-level provenance, the release manifest records the identities and
content hashes of the archived integration-level cube and reporting mask. It
also records the code, configuration, calibration and response products,
source and background products, detector arrays, final predictions, and score
tables. Large cluster-only arrays may be represented in the portable archive
by authenticated manifests. Superseded, diagnostic, unsuccessful, and
prospective branches retain an explicit status so that they cannot be mistaken
for the reported method.
