...
truth or any recovered-depth residual. This assessment evaluates a fixed
implementation of NOVA, termed NOVA-S, whose exact specification and
provenance are given in Appendix~\ref{app:nova}.
...
the order-discrepancy penalty. The penalty is retained in the objective;
what pins $\Delta$ to zero in NOVA-S is its numerical bounds
...
fails closed. The synthetic event occupies integrations 51--177, leaving
102 event-complement integrations. The fixed calibration masks are
selected from this complement as detailed in Appendix~\ref{app:nova}. The full construction, delivery manifest, and
...
contains no groups. A preregistered paired experiment will therefore inject
the same deficit at group stage and at integration stage and run NOVA and
