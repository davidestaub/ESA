...
improve each order individually; the per-order candidate improved the common score by only 1.85\%,
short of the aggregate requirement, and its order-2 score (1.0226) was
worse than the common counterpart (0.99964), so the common factor was
...
physically valid improvement beyond the relative material threshold, whose
executable value is the relative tolerance scaled by
...
confirmation uses $10^{-10}$ and 12. An accepted fit must
